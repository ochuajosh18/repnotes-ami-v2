...
export function fieldsReducer(state = INITIAL_STATE, action: FieldsMngtAction): FieldsMngtState {
  switch (action.type) {
    ...
    case ADD_FIELD: {
      ...
      let row = 1;
      if (key === "addedFields") {
        const targetFieldModule = state.data.find((d) => d.id === fieldModuleId);
        if (targetFieldModule) {
          const addedFieldsLength = targetFieldModule.addedFields.length;
          row =
            addedFieldsLength === 0
              ? 1
              : targetFieldModule.addedFields.filter((f) => f.column === 1).length + 1;
        }
      }
      return {
        ...state,
        data: state.data.map((d) =>
          d.id === fieldModuleId ? { ...d, [key]: [...d[key], { ...field, row }] } : d
        ),
      };
    }
    case EDIT_FIELD: {
